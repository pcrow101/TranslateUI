...
/// Decides whether each recognised block is German, Italian or English.
///
...
nonisolated struct LanguageDetector: Sendable {
    private static let candidates: [NLLanguage: SourceLanguage] = [
        .german: .german,
        .italian: .italian,
        .english: .english
    ]
    ...
    private nonisolated func language(of text: String) -> (language: SourceLanguage, confidence: Double) {
        ...
        recognizer.languageConstraints = Array(Self.candidates.keys)
        recognizer.processString(trimmed)

        let hypotheses = recognizer.languageHypotheses(withMaximum: 3)
        guard
            let best = hypotheses.max(by: { $0.value < $1.value }),
            let mapped = Self.candidates[best.key]
        else {
            return (.unknown, 0)
        }
